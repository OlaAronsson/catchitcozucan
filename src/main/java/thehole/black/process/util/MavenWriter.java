package thehole.black.process.util;
...
public class MavenWriter {
	...
	public static String formattedErrors(String in) {
		StringBuilder formatted = new StringBuilder(NL).append(NL).append(in.substring(0, in.indexOf(COLON))).append(NL);
		...
		Arrays.stream(messages).forEach(m -> {
			formatted.append(NL).append(INLINE_NEW_ERR);
			...
			for (int i = 0; i < chars.length; i++) {
				...
				if (inRowPos < MAX_ERR_MSG_LEN + 1) {
					...
					formatted.append(NL).append(INLINE);
					inRowPos = -1;
					i = i + charsToGrab;
				}
			}
