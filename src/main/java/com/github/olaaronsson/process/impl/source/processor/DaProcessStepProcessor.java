...
import static com.github.olaaronsson.process.impl.source.processor.DaProcessStepConstants.ANNOT_PROCESSSTATUS_JAVA_PATH;
import static com.github.olaaronsson.process.impl.source.processor.DaProcessStepConstants.DOT;
import static com.github.olaaronsson.process.impl.source.processor.DaProcessStepConstants.EMPTY;
import static com.github.olaaronsson.process.impl.source.processor.DaProcessStepConstants.error;
import static com.github.olaaronsson.process.impl.source.processor.DaProcessStepConstants.info;
import static com.github.olaaronsson.process.impl.source.processor.DaProcessStepConstants.warn;
import static com.github.olaaronsson.process.impl.source.processor.DaProcessStepLookup.TMP_COMP_PATH;
...
import com.github.olaaronsson.process.internal.util.MD5Digest;
import com.github.olaaronsson.process.internal.util.io.IO;
import com.github.olaaronsson.process.util.ClassAnnotationUtil;
import com.github.olaaronsson.process.util.MavenWriter;

@SupportedAnnotationTypes({ "com.github.olaaronsson.process.MakeStep" })
public class DaProcessStepProcessor extends AbstractProcessor {
	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) { //NOSONAR
		...
		if (roundEnv.processingOver()) {
			...
			IO.deleteRecursively(new File(TMP_COMP_PATH));
			return true;
		}
	...
	private void testLoadStatuses(StringBuilder errors, String enumProvider, String statusUponSuccess, String statusUponFailure, DaProcessStepSourceAppender appender) { //NOSONAR
		...
		if (goOnEvaluating) {
			...
			error(String.format("    Failed inspecting Makestep enum definitions based on class %s!", enumProvider));
		}
	}
	...
	private static Class<?> loadStatusClassBySheerForceTmpCompileFromFile(File file) {
		File tmpDir = new File(TMP_COMP_PATH);
		...
		if (tmpDirIsAvail) {
			...
			try {
				...
				if (source.contains(PACKAGE)) {
					...
					source = source.replace(packageDecl, EMPTY);
				}
				...
				if (!processStatusImportFound || source.indexOf(IMPORT, (importIndex + importDecl.length())) > -1) {
					error("Your status class cannot contain ANY other import that of ths ProcessStatus annotation. It is not allowed.");
					return null;
				}
				source = source.replace(importDecl, "").replace("@ProcessStatus", "");
				file = new File(TMP_COMP_PATH + File.separator + file.getName());
