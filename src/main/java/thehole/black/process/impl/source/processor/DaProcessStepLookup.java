...
import static thehole.black.process.impl.source.processor.DaProcessStepConstants.ANNOT_MAKESTEP_JAVA_PATH;
import static thehole.black.process.impl.source.processor.DaProcessStepConstants.ANNOT_PROCESSSTATUS_JAVA_PATH;
import static thehole.black.process.impl.source.processor.DaProcessStepConstants.NEN_BLACK_PROCESS_MAKESTEP_SUPPORTED_TYPES;
import static thehole.black.process.impl.source.processor.DaProcessStepConstants.OWNER;
import static thehole.black.process.impl.source.processor.DaProcessStepConstants.TSYM;
import static thehole.black.process.impl.source.processor.DaProcessStepConstants.TYPE;
...
import thehole.black.process.ProcessStatus;
import thehole.black.process.internal.util.io.IO;
import thehole.black.process.internal.util.reflect.ReflectionUtils;
...
public class DaProcessStepLookup {
	...
	public static Set<DaProcessStepSourceAppender> annotatedElementsToAppenders(Set<? extends Element> annotatatedElementsMakeStep, Set<? extends Element> annotatatedElementsProcessStatus) { // NOSONAR
		...
		annotatatedElementsMakeStep.stream().forEach(annotatedElement -> {
			...
			if (isValidMethod(toExec, annotationTypePath)) {
				...
				Object classSymbol = ReflectionUtils.getFieldValueSilent((ReflectionUtils.getFieldValueSilent(ReflectionUtils.getFieldValueSilent(annotatedElement, OWNER), TYPE)), TSYM);
		...
		annotatatedElementsProcessStatus.stream().forEach(annotatedElement -> classSymbols.add(ReflectionUtils.getFieldValueSilent((ReflectionUtils.getFieldValueSilent(annotatedElement, TYPE)), TSYM))); // SONAR..any good?
	...
	private static boolean isValidMethod(ExecutableElement item, String annotationTypePath) { //NOSONAR
		...
		if (annotationTypePath.equals(ANNOT_MAKESTEP_JAVA_PATH)) {
			...
			NEN_BLACK_PROCESS_MAKESTEP_SUPPORTED_TYPES.stream().forEach(c -> DaProcessStepConstants.info("    *  " + c));
			return false;
		}
	}
